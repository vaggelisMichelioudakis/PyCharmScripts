...
FOLDER = r"Digitized_TC_CSV"   # <-- ΒΑΛΕ ΕΔΩ ΤΟ PATH ΣΟΥ
...
def parse_state_and_base(filename_no_ext: str):
    ...
    if "_COLD" in name_u:
        return "COLD", re.split(r"_COLD", filename_no_ext, flags=re.IGNORECASE)[0]
    if "_HOT" in name_u:
        return "HOT", re.split(r"_HOT", filename_no_ext, flags=re.IGNORECASE)[0]
    return None, None
...
keys = sorted([k for k, v in systems.items() if "COLD" in v and "HOT" in v])
print(len(keys))
if not keys:
    raise RuntimeError("Δεν βρέθηκαν ζευγάρια COLD/HOT. Έλεγξε ονόματα αρχείων και φάκελο.")
...
for i, k in enumerate(keys):
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="System: %{legendgroup}<br>State: COLD<br>λ=%{x} nm<br>T=%{y:.2f}%<extra></extra>"
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="System: %{legendgroup}<br>State: HOT<br>λ=%{x} nm<br>T=%{y:.2f}%<extra></extra>"
    ))

fig.update_layout(
    title="Transmittance spectra per system (COLD solid / HOT dashed)",
